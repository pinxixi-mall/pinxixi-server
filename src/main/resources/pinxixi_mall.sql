...
CREATE TABLE `tb_carousel`(
    ...
    `carousel_image` varchar(100) NOT NULL COMMENT '轮播图片地址',
    `carousel_url` varchar(100) NOT NULL COMMENT '轮播图跳转链接',
    `carousel_sort` int(11) NOT NULL DEFAULT 0 COMMENT '轮播图排序（越小越靠前）',
    `is_deleted` tinyint(4) NOT NULL DEFAULT 0 COMMENT '是否已删除（0-否，1-是）',
    `create_user` int(11) NOT NULL COMMENT '创建人id',
    `create_time` datetime NOT NULL CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_user` int(11) NOT NULL COMMENT '更新人id',
    `update_time` datetime NOT NULL CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `tb_goods` (
    ...
    `goods_image` varchar(200) NOT NULL DEFAULT '' COMMENT '商品主图',
    `goods_category_id` bigint(20) NOT NULL DEFAULT 0 COMMENT '商品分类id',
    `goods_desc` varchar(200) NOT NULL DEFAULT '' COMMENT '商品简介',
    `goods_price` float(11) NOT NULL DEFAULT '6.6' COMMENT '商品价格',
    `goods_stock` int(11) unsigned NOT NULL DEFAULT 0 COMMENT '商品库存',
    `goods_status` tinyint(4) NOT NULL DEFAULT 1 COMMENT '商品状态（0-下架，1-上架）',
    `goods_type` tinyint(4) NOT NULL DEFAULT 0 COMMENT '商品类型（0-普通，1-推荐）',
...
CREATE TABLE `tb_client_order` (
    ...
    `order_price` int(11) NOT NULL DEFAULT '0' COMMENT '订单价格',
    `payment_status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '支付状态（0-未支付，1-支付成功，1-支付失败）',
    `order_status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '订单状态（0-待支付，1-已支付，2-配货完成，3-出库成功，4-交易成功，5-手动关闭，6-超时关闭，7-商家关闭，99-已删除）',
    `pay_type` tinyint(4) NOT NULL DEFAULT '0' COMMENT '支付类型（1-支付宝，2-微信）',
...
CREATE TABLE `tb_client_cart` (
    ...
    `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`cart_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客户购物车表';
...
CREATE TABLE `tb_client_address` (
    ...
    `address_detail` varchar(64) NOT NULL DEFAULT '' COMMENT '详细地址',
    `is_deleted` tinyint(4) NOT NULL DEFAULT '0' COMMENT '删除标识（0-未删除，1-已删除）',
    `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
